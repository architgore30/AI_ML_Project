...
import pipeline_config as pipeline_config
...
INPUT_PATH = pipeline_config.LABELED_PATH
OUTPUT_PATH = pipeline_config.FEATURES_PATH
SHOW_PROGRESS = True
...
def build_feature_dataframe(df, show_progress=False, progress=None):
    ...
    try:
        ...
        atr_abs_14 = working["atr_pct_14"] * close
        working["macd_line_pct_close"] = safe_divide(macd_line, close)
        working["macd_signal_pct_close"] = safe_divide(macd_signal, close)
        working["macd_hist_pct_close"] = safe_divide(macd_histogram, close)
        working["macd_hist_atr"] = safe_divide(macd_histogram, atr_abs_14)
        working["log_close"] = np.where(close > 0, np.log(close), np.nan)
        working["log_volume"] = np.log1p(volume.clip(lower=0))
        progress.done()
